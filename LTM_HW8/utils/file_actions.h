...
void rewrite_file()
{
  ...
  while (p != NULL)
  {
    fprintf(f, "%s %s %d %s\n", p->data.username, p->data.password, p->data.status, p->data.homepage);
    p = p->next;
  }
...
void display_linked_list()
{
  ...
  while (p != NULL)
  {
    ...
    printf("%-5d%-20s%-25s%-25s%-25s\n", i + 1, p->data.username, p->data.password, status, p->data.homepage);
    i++;
    p = p->next;
  }
...
void read_from_file()
{
  ...
  while (fscanf(f, "%s%s%d%s", a[i].username, a[i].password, &a[i].status, a[i].homepage) != EOF)
  {
